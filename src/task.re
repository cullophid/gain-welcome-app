external resolvePromise : Js.Promise.t 'a => ('a => unit) => ('err => unit) => unit = "then" [@@bs.send];
...
let map f task => make (fun fail success => {
task fail (fun x => f x  |> success)
});

let chain f task => make (fun fail success => {
  task fail (fun x => (f x) fail success)
});

let biMap mapFail mapSuccess task => make (fun fail success => {
